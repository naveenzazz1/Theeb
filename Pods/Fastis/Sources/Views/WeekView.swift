...
class WeekView: UIView {
    ...
    // MARK: - Lifecycle
    init(config: FastisConfig.WeekView,isArabic: Bool) {
        self.config = config
        super.init(frame: .zero)
        self.configureUI()
        self.configureSubviews(isArabic: isArabic)
    ...
    private func configureSubviews(isArabic:Bool) {
        config.calendar.locale = Locale(identifier: isArabic ? "ar":"en")
        var weekDays = self.config.calendar.shortWeekdaySymbols
        weekDays.append(weekDays.remove(at: 0))
        for weekdaySimbol in isArabic ? weekDays.reversed():weekDays {
