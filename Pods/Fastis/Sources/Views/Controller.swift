...
public class FastisController<Value: FastisValue>: UIViewController, JTACMonthViewDelegate, JTACMonthViewDataSource {
    ...
    public var isArabic = true{
        didSet{
            currentValueView.isArabic = isArabic
            appearance.cancelButtonTitle = isArabic ? "عودة":"Back"
            config.currentValueView.placeholderTextForRanges = isArabic ? "حدد النطاق الزمني":"Select date range"
        }
    }
    private lazy var weekView: WeekView = {
        return WeekView(config: self.config.weekView, isArabic: isArabic)
    }()
    ...
    public override func viewWillAppear(_ animated: Bool) {
        ...
        [weekView,calendarView,currentValueView].forEach{
    ...
    */
    ...
    public var secondHandler: ((Value?) -> Void)?
    public var didEndSelectRange: ((Value?) -> Void)?
    ...
    */
    ...
    public func calendar(_ calendar: JTACMonthView, didSelectDate date: Date, cell: JTACDayCell?, cellState: CellState, indexPath: IndexPath) {
        if cellState.selectionType == .some(.userInitiated) {
            self.didEndSelectRange?(self.value)
...
extension FastisConfig {
    public struct Controller {
        ...
        public var cancelButtonTitle: String? = "رجوع"
