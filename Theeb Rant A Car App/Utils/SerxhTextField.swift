...
open class SearchTextField: UITextField {
    ...
    fileprivate let indicator = UIActivityIndicatorView(style: .medium)//UIActivityIndicatorView(style: .gray)
    ...
    fileprivate func prepareDrawTableResult() {
        guard let frame = self.superview?.convert(self.frame, to: UIApplication.keyWindow) else { return }
        if let keyboardFrame = keyboardFrame {
            ...
            if self.center.y + theme.cellHeight > UIApplication.keyWindow!.frame.size.height {
                direction = .up
            } else {
                direction = .down
            }
